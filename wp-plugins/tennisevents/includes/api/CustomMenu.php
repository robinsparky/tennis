 ...
 * Class CustomMenu
 *
 * This class is for creating and managing dynamic menu items in a WordPress menu.
 * Based on custom_menu_items from Jonathan Daggerhart
 * @see https://www.daggerhart.com/dynamically-add-item-to-wordpress-menus/
 */
class CustomMenu {
	/**
	...
	function wp_get_nav_menu_items( $items, $menu ) {
		...
		$this->log->error_log("$loc: menu slug='{$menu->slug}' ($ctr)");
		++$ctr;
	...
	static public function add_item( $menu_slug, $title, $url, $order = 0, $parent = 0, $ID = null, $classes = array() ){
		$instance = CustomMenu::get_instance();
